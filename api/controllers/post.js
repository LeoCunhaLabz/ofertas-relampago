    ...
    db.query("SELECT moedas FROM anunciantes WHERE email = ?", [email], async (error, data) => {
        ...
        if (saldo_pre < 1) {
            ...
            try {
                ...
                // Inserir produto
                db.query("INSERT INTO anuncios SET ?", newProduct, (error, result) => {
                    if (error) {
                        console.log(error);
                        return res.status(500).json({ message: "Erro ao inserir produto." });
                    } else {
                        const id_anuncio = result.insertId;
                        try {
                            // Atualizar saldo
                            db.query("UPDATE anunciantes SET moedas = moedas - 1 WHERE email = ?", [email], async (error) => {
                                if (error) {
                                    console.log(error);
                                    return res.status(500).json({ message: "Erro ao atualizar saldo." });
                                } else {
                                    const transacao = {
                                        data_hora_transacao: data_cadastro,
                                        num_creditos: -1,
                                        tipo_transacao: 'postagem_anuncio',
                                        id_anunciante,
                                        id_anuncio,
                                        descricao: nome_produto,
                                        saldo: saldo_pre - 1,
                                    };
                                    db.query("INSERT INTO transacoes_pj SET ?", transacao, (error) => {
                                        if (error) {
                                            console.log(error);
...
export const getAllOfertas = (req, res) => {
...
export const getOneOferta = (req, res) => {
    const { email, oferta_id, latitude, longitude } = req.body;
    if (email !== process.env.EMAIL_ADMIN) {
        return res.status(400).json({ message: "Usuário não autorizado." });
    } else if (email === process.env.EMAIL_ADMIN) {
        db.query(
...
export const getTransactions = async (req, res) => {
    ...
    // Verificar se campos foram preenchidos
    if (!email || !id) {
        return res.status(400).json({ message: "Por favor, forneça o email e o id." });
    }

    // Consultar transações
    db.query("SELECT * FROM transacoes_pj WHERE id_anunciante = ?", [id], (error, results) => {
        if (error) {
            console.log(error);
