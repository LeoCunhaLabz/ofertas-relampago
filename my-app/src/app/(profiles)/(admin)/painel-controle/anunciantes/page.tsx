...
export default function PainelControleClientes() {
    ...
    return (
        <main className="w-full mt-0">
            <Title className="text-center">Total de Anunciantes: {events.length} </Title>
                    ...
                    <tbody>
                        {currentEvents?.map((event) => (
                            <tr key={event.id}>
                                ...
                                <td className={`border px-4 py-2 ${Number(event.habilitado) === 1 ? 'text-green-500' : 'text-red-500'}`}>
                                    {Number(event.habilitado) === 1 ? "Cadastrado" : "Anunciante Negado"}
                                </td>
                            </tr>
                        ))}
                    </tbody>
